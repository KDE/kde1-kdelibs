...
macro (QT1_MAKE_OUTPUT_FILE infile prefix ext outfile )
...
macro (QT1_GET_MOC_FLAGS _moc_flags)
  set(${_moc_flags})
...
function (QT1_CREATE_MOC_COMMAND infile outfile moc_flags moc_options moc_target)
  ...
  message( STATUS "------- ${_moc_parameters_file} ${_moc_parameters}" )
  ...
  add_custom_command(OUTPUT ${outfile}
                      COMMAND /opt/qt1/bin/moc ${_moc_extra_parameters_file}
...
macro (QT1_GENERATE_MOC infile outfile )
# get include dirs and flags
   QT1_GET_MOC_FLAGS(moc_flags)
   ...
   QT1_CREATE_MOC_COMMAND(${abs_infile} ${_outfile} "${moc_flags}" "" "${moc_target}")
   set_source_files_properties(${outfile} PROPERTIES SKIP_AUTOMOC TRUE)  # dont run automoc on this file
endmacro ()


# QT1_WRAP_CPP(outfiles inputfile ... )

macro (QT1_WRAP_CPP outfiles )
  # get include dirs
  QT1_EXTRACT_OPTIONS(moc_files moc_options moc_target ${ARGN})

  foreach (it ${moc_files})
    get_filename_component(it ${it} ABSOLUTE)
    QT1_MAKE_OUTPUT_FILE(${it} moc_ cxx outfile)
    QT1_CREATE_MOC_COMMAND(${it} ${outfile} "${moc_flags}" "${moc_options}" "${moc_target}")
    set(${outfiles} ${${outfiles}} ${outfile})
  endforeach()
endmacro ()

# QT1_WRAP_UI(outfiles inputfile ... )

macro (QT1_WRAP_UI outfiles )
...
# QT1_ADD_RESOURCES(outfiles inputfile ... )

macro (QT1_ADD_RESOURCES outfiles )
  QT1_EXTRACT_OPTIONS(rcc_files rcc_options rcc_target ${ARGN})
  ...
  foreach (it ${rcc_files})
    ...
    if(EXISTS "${infile}")
      ...
      QT1_MAKE_OUTPUT_FILE("${infile}" "" "qrc.depends" out_depends)
...
macro(QT1_ADD_DBUS_INTERFACE _sources _interface _basename)
...
macro(QT1_ADD_DBUS_INTERFACES _sources)
  foreach (_current_FILE ${ARGN})
    ...
    QT1_ADD_DBUS_INTERFACE(${_sources} ${_infile} ${_basename}interface)
  endforeach ()
endmacro()


macro(QT1_GENERATE_DBUS_INTERFACE _header) # _customName OPTIONS -some -options )
  QT1_EXTRACT_OPTIONS(_customName _qt1_dbus_options _qt1_dbus_target ${ARGN})
  ...
  add_custom_command(OUTPUT ${_target}
      COMMAND Qt4::qdbuscpp2xml ${_qt1_dbus_options} ${_in_file} -o ${_target}
      DEPENDS ${_in_file} VERBATIM
  )
endmacro()


macro(QT1_ADD_DBUS_ADAPTOR _sources _xml_file _include _parentClass) # _optionalBasename _optionalClassName)
...
macro(QT1_AUTOMOC)
  if(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 2.8.11)
    message(DEPRECATION "The qt1_automoc macro is obsolete. Use the CMAKE_AUTOMOC feature instead.")
  endif()
  QT1_GET_MOC_FLAGS(_moc_INCS)
    ...
    if ( NOT _skip AND EXISTS ${_abs_FILE} )
      ...
      if(_match)
        foreach (_current_MOC_INC ${_match})
          ...
          QT1_CREATE_MOC_COMMAND(${_header} ${_moc} "${_moc_INCS}" "" "")
...
macro(QT1_CREATE_TRANSLATION _qm_files)
   QT1_EXTRACT_OPTIONS(_lupdate_files _lupdate_options _lupdate_target ${ARGN})
   set(_my_sources)
   set(_my_dirs)
   set(_my_tsfiles)
   set(_ts_pro)
   ...
   endforeach()
   QT1_ADD_TRANSLATION(${_qm_files} ${_my_tsfiles})
endmacro()


macro(QT1_ADD_TRANSLATION _qm_files)
  foreach (_current_FILE ${ARGN})
    get_filename_component(_abs_FILE ${_current_FILE} ABSOLUTE)
...
function(qt1_use_modules _target _link_type)
  if(NOT CMAKE_MINIMUM_REQUIRED_VERSION VERSION_LESS 2.8.11)
    message(DEPRECATION "The qt1_use_modules function is obsolete. Use target_link_libraries with IMPORTED targets instead.")
  endif()
  ...
  foreach(_module ${modules})
    ...
    if (_ucmodule STREQUAL QAXCONTAINER OR _ucmodule STREQUAL QAXSERVER)
      ...
      if ("${_ucmodule}" STREQUAL "MAIN")
        message(FATAL_ERROR "Can not use \"${_module}\" module with qt1_use_modules.")
      endif()
    endif()
