...
int createCollection(char *name=NULL); // Returns the id associated to the 
                           // new collection
                           // -1 if name already is used and Collection was
                           //  not created

void deleteCollection(int id);
void changeCollectionName(int id,char *newname);

SongList *getCollection(int id);
SongList *getCollection(char *name);
char *getCollectionName(int id);
